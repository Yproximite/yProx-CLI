...
const globals = require('rollup-plugin-node-globals-ecmaversion-9');
  ...
  const watch = () => {
    ...
    watcher.on('event', (e) => {
      ...
      if (code === 'BUNDLE_START') {
        log.info(`[watch-rollup]: start bundling "${entry.src}"`);
      } else if (code === 'BUNDLE_END') {
        log.info(`[watch-rollup]: done bundling "${entry.src}"`);
      } else if (['ERROR', 'FATAL'].includes(code)) {
        log.error(`[watch-rollup]: something wrong happens`);
        log.error(e);
      }
    });
  };
