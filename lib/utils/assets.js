...
const readEntryFile = (api, entryName, entryFile) => {
  const resolvedEntryFile = resolve(api.context, entryFile);

  return require(resolvedEntryFile)(api, api.projectOptions)
    .map(entry => {
      entry._name = entryName;
      entry.dest = resolve(api.context, entry.dest);
      return entry;
    });
};
